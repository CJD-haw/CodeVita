...
The input format for testing
    ...
    First input     – Accept value for N(Positive integer) values (a[]), where each value is separated by a new line.
...
fine = int(input())
if fine not in range(100, 5001):
    print('Invalid Input')
    exit()

if date % 2 == 0:
    odd = [val for val in digit if val % 2 != 0]
    fine *= len(odd)
else:
    even = [val for val in digit if val % 2 == 0]
    fine *= len(even)
