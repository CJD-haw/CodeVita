...
Explanation:
    2 members should always be next to each other.
    So, 2 members can be in 2! ways
    Rest of the members can be arranged in (10-1)! Ways.
    (1 is subtracted because the previously selected two members will be considered as a single member now).
    ...
    (Check the output in example 1, example2)
...
number = int(input())
if not (number < 2 or number > 50):
    fact = [n for n in range(1, number)]
    result = 2
    for val in fact:
        result *= val
    print(result)
