...
Those numbers are studied thoroughly.
It is concluded that all digits of the number ‘N’ are summed up and this action is performed ‘R’ number of times.
...
number = int(input())
repeat = int(input())
total = sum([int(val) for val in str(number)])
temp = total * repeat
result = sum([int(val) for val in str(temp)])
print(result)
