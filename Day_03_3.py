...
Note:
    Kindly follow  the order of inputs as n,k,j,m,p as given in the above example.
    And output must include  the same format  as in above example(Number of Monkeys left on the Tree:)
    For any wrong input display INVALID INPUT
"""

n = int(input())    # Number of monkeys on the tree are
k = int(input())    # Number of bananas for one monkey
j = int(input())    # Number of peanuts for one monkey
m = int(input())    # Number of Bananas
p = int(input())    # Number of peanuts

if n > 1 and k != 0 and j != 0 and m % k == 0 and p % j == 0 and m >= k and p >= j:
    flag = True
    while flag:
        if m:
            m -= k
            ...
            flag = False
    print('Number of monkey left on tree are {}'.format(n))
else:
    print('Invalid Input')
